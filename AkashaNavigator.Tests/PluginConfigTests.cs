...
public class PluginConfigTests
{
    ...
    public Property SetThenGet_ShouldReturnSameValue_ForStrings(NonEmptyString key, NonEmptyString value)
    {
        // 排除包含空白的键（无效键）
        ...
        return (retrieved == value.Get).When(validKey).Label($"Set '{key.Get}' = '{value.Get}', Get = '{retrieved}'");
    }
    ...
    /// </summary>
    [Property(MaxTest = 100)]
    public Property SetThenGet_ShouldReturnSameValue_ForIntegers(NonEmptyString key, int value)
    {
        var validKey = !string.IsNullOrWhiteSpace(key.Get) && !key.Get.Contains(' ');
        ...
        return (retrieved == value).When(validKey).Label($"Set '{key.Get}' = {value}, Get = {retrieved}");
    }
    ...
    /// </summary>
    [Property(MaxTest = 100)]
    public Property SetThenGet_ShouldReturnSameValue_ForBooleans(NonEmptyString key, bool value)
    {
        var validKey = !string.IsNullOrWhiteSpace(key.Get) && !key.Get.Contains(' ');
    ...
    /// </summary>
    [Property(MaxTest = 100)]
    public Property SetThenGet_ShouldReturnSameValue_ForDoubles(NonEmptyString key, NormalFloat value)
    {
        var validKey = !string.IsNullOrWhiteSpace(key.Get) && !key.Get.Contains(' ');
        ...
        return isEqual.When(validKey).Label($"Set '{key.Get}' = {value.Get}, Get = {retrieved}");
    }
    ...
    /// </summary>
    [Property(MaxTest = 100)]
    public Property SetThenGet_ShouldWork_WithDotNotationPath(NonEmptyString parent, NonEmptyString child, int value)
    {
        // 排除包含空白或点号的键部分
        var validParent =
            !string.IsNullOrWhiteSpace(parent.Get) && !parent.Get.Contains(' ') && !parent.Get.Contains('.');
        var validChild = !string.IsNullOrWhiteSpace(child.Get) && !child.Get.Contains(' ') && !child.Get.Contains('.');
        ...
        return (retrieved == value).When(validParent && validChild).Label($"Set '{key}' = {value}, Get = {retrieved}");
    }
    ...
    /// </summary>
    [Property(MaxTest = 50)]
    public Property SaveAndLoad_ShouldPreserveValues(NonEmptyString pluginId, NonEmptyString key, int value,
                                                     bool enabled)
    {
        ...
        try
        {
            var config = new PluginConfig(pluginId.Get) { Enabled = enabled };
            config.Set(key.Get, value);
